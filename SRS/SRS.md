...
### 2.3用户类型和特征
详细的用户类型和特征可在本文档目录1.1目的中找到，此处不再重复。
### 2.4操作环境
#### 硬件平台、操作系统及其版本：
...
- 用户分布：中国境内
- 服务器位置：数据中心位于纽约、伦敦和新加坡，以提供全球范围内的高可用性。
- 数据库位置：主数据库在纽约，备份数据库在伦敦。

#### 托管相关数据库、服务器和网站的组织:

- 数据库托管组织：Amazon Web Services (AWS) RDS
- 服务器托管组织：Microsoft Azure
- 网站托管组织：Google Cloud Platform (GCP)
...
病人可以自己设置运动方案，记录在表scenario中，每人最多只能拥有1个运动方案，只能通过选择系统推荐方案或者自己去修改方案。病人可以定期体检，体检结果记录在表examine中。
